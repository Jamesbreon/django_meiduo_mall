...
class GoodsCategory(BaseModels):
    ...
    # 由于与GoodChannel有外键管关系
    # 可以认为在GoodCategory产生 goodschannel_set 字段
...
class GoodsChannel(BaseModels):
    """商品频道"""
    group = models.ForeignKey(GoodsChannelGroup, verbose_name='')
...
class SPU(BaseModels):
    ...
    # sku_qs = categpry.sku_set.filter(is_launched=True).order_by('-sales')
...
class GoodsVisitCount(BaseModels):
    ...
    date = models.DateField(auto_now_add=True, verbose_name='统计时间')
