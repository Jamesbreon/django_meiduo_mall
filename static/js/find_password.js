var vm = new Vue({
    ...
    methods: {
            ...
            if (this.error_username == false && this.error_image_code == false) {
                axios.get(this.host + '/accounts/' + this.username + '/sms/token/?image_code=' + this.image_code + '&image_code_id=' + this.image_code_id, {
                    ...
                    .then(response => {
                        ...
                        this.image_code = response.data.image_code;
        ...
        send_sms_code: function () {
            ...
            axios.get(this.host + '/sms_codes/' + this.mobile + '/?image_code=' + this.image_code + '&uuid=' + this.access_token, {
            responseType: 'json'
        })
    .then(response => {
        // 表示后端发送短信成功
...
form_2_on_submit: function () {
    ...
    if (this.error_sms_code == false) {
        axios.get(this.host + '/accounts/' + this.username + '/password/token/?sms_code=' + this.sms_code + '&access_token='+ this.access_token, {
            responseType: 'json'
        })
            .then(response => {
